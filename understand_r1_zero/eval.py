...
def save_summary_csv(results, avg_lens, max_lens, formatted, filename):
    """Save summary results to CSV with one row per task."""
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['task_name', 'accuracy', 'avg_length', 'max_length', 'formatted_rate']
        ...
        for task_name in results.keys():
            writer.writerow({
                ...
                'avg_length': avg_lens.get(task_name, ''),
                'max_length': max_lens.get(task_name, ''),
                'formatted_rate': formatted.get(task_name, '')
            })

        # Add overall average row
        writer.writerow({
            'task_name': 'OVERALL_AVERAGE',
            'accuracy': np.mean(list(results.values())),
            'avg_length': np.mean(list(avg_lens.values())) if avg_lens else '',
            'max_length': np.max(list(max_lens.values())) if max_lens else '',
            'formatted_rate': np.mean(list(formatted.values())) if formatted else ''
        })


def save_detailed_csv(to_be_saved, filename):
    """Save detailed results to CSV with one row per sample."""
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['task_name', 'sample_idx', 'prompt', 'ground_truth',
                     'model_output', 'reward', 'output_length']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for item in to_be_saved:
            # Since each item can have multiple outputs (n_samples), create a row for each
            for idx, (output, gt, reward, token_ids) in enumerate(zip(
                item['model_output'],
                item['gt'],
                item['reward'],
                item['model_output_token_ids']
            )):
                writer.writerow({
...
def main(
    ...
    max_test: int = 999999,
    save: bool = False,
    save_csv: bool = True,  # New parameter to control CSV saving
    save_detailed_csv: bool = False,  # New parameter to save detailed CSV
):
    ...
    # Save results
    if save or save_csv or save_detailed_csv:
        base_fn = "model_eval_out_" + model_name.replace("/", "_") + str(int(time.time()))
        base_fn = f"{base_fn}_template_{template}_temp{temperature}_topp{top_p}_n{n_samples}"

        # Save JSON if requested
        if save:
            json_fn = f"{base_fn}.json"
            print(f"saving model outputs at {json_fn}")
            json.dump(
                to_be_saved,
                open(json_fn, "w"),
                indent=4,
        ...
        # Save summary CSV
        if save_csv:
            summary_csv_fn = f"{base_fn}_summary.csv"
            print(f"saving summary results to {summary_csv_fn}")
            save_summary_csv(results, avg_lens, max_lens, formatted, summary_csv_fn)

        # Save detailed CSV if requested
        if save_detailed_csv:
            detailed_csv_fn = f"{base_fn}_detailed.csv"
            print(f"saving detailed results to {detailed_csv_fn}")
            save_detailed_csv(to_be_saved, detailed_csv_fn)
